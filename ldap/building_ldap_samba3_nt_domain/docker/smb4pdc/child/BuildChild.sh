...
echo "docker build --build-arg SMB_REALM=$SMB_REALM --build-arg SMB_DNS_BACKEND=$SMB_DNS_BACKEND --build-arg SMB_DOMAIN=$SMB_DOMAIN --build-arg SMB_HOSTNAME=$SMB_HOSTNAME --build-arg SMB_ADMIN_PASS=$SMB_ADMIN_PASS -t=${SMB_AUTHOR_NAME}/inner_smb4pdc ."
...
docker run --name "inner_smb4pdc" -t -i ${SMB_AUTHOR_NAME}/inner_smb4pdc
